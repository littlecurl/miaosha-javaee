...
import com.example.mavendemo.dao.OrderDOMapper;
import com.example.mavendemo.dao.SequenceDOMapper;
import com.example.mavendemo.dataobject.OrderDO;
...
//        orderModel.setOrderPrice(itemModel.getPrice().multiply(new BigDecimal(amount.intValue())));
        orderModel.setOrderPrice(orderModel.getItemPrice().multiply(new BigDecimal(amount.intValue())));
    ...
    public String generateOrderNo(){
        ...
        String squenceStr = String.valueOf(sequence);
        for (int i=0; i<6-squenceStr.length(); i++){
            stringBuilder.append(0);
        }
        stringBuilder.append(squenceStr);
    ...
    private OrderDO convertFromOrderModel(OrderModel orderModel){
        ...
        // 因为dataobject中的price是Double的，而数据库中的price是Decimal的
        // 所以需要手动设值
